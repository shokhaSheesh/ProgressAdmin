    ...
    <ModalBackdrop onClose={onClose}>
      <div className="bg-card rounded-2xl w-[460px] max-w-full overflow-hidden" style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.2)' }}>
        <div className="flex flex-col items-center gap-4 px-8 pt-8 pb-6 text-center">
          ...
          <div>
            <p className="text-[18px] font-extrabold text-foreground">Delete Shop</p>
...
function FormModal({ shop, onClose, onSave }: { shop: Shop | null; onClose: () => void; onSave: (d: FormState) => void }) {
  ...
  return (
    <ModalBackdrop onClose={onClose}>
      <div className="bg-card rounded-2xl w-[640px] max-w-full max-h-[90vh] overflow-y-auto" style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.2)' }}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-black/[0.06] sticky top-0 bg-card z-10 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[16px] font-bold text-foreground">{isEdit ? 'Edit Shop' : 'Add New Shop'}</p>
              <p className="text-[12px] font-medium text-muted-foreground">{isEdit ? `Editing ${shop.name}` : 'Fill in the details below'}</p>
            </div>
          </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (form.name.trim() && form.ownerName.trim() && form.ownerPhone.trim() && form.ownerLogin.trim()) onSave(form) }} className="p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Shop Name</label>
            <input type="text" value={form.name} onChange={(e) => set('name')(e.target.value)} placeholder="e.g. AutoZone Tashkent" required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Owner Name</label>
              <input type="text" value={form.ownerName} onChange={(e) => set('ownerName')(e.target.value)} placeholder="e.g. Bekzod Saidov" required
                className="w-full bg-[#F4F5F7] text-foreground rounded-xl px-4 py-3 text-[13px] font-medium border border-transparent focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" value={form.address} onChange={(e) => set('address')(e.target.value)} placeholder="e.g. Yunusabad, 7-mavze, Tashkent"
                className="w-full bg-[#F4F5F7] text-foreground rounded-xl px-4 py-3 text-[13px] font-medium border border-transparent focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>
            <div className="flex flex-col gap-1.5">
      ...
      <div className="bg-card rounded-2xl border border-black/[0.06] overflow-hidden" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
        ...
        <div className="px-5 py-3.5 border-b border-black/[0.06] flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" value={search} onChange={(e) => handleSearch(e.target.value)} placeholder="Search by shop, owner, address…"
          ...
          <table className="w-full">
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                return (
                  <tr key={shop.id} className="border-b border-black/[0.04] hover:bg-[#F4F5F7]/70 transition-colors last:border-0">
                    {/* Shop name */}
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <span className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-white text-[11px] font-bold ${ab}`}>
                          {initials(shop.name)}
                        </span>
                        <span className="text-[13px] font-semibold text-foreground whitespace-nowrap">{shop.name}</span>
                      </div>
                    </td>
                    {/* Owner */}
                    ...
                    {/* Created At */}
                    <td className="px-5 py-3.5 text-[13px] font-medium text-muted-foreground whitespace-nowrap">{shop.createdAt}</td>
                    {/* Actions */}
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1">
                        <button title="Edit" onClick={() => setModal({ kind: 'edit', shop })} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-[#F4F5F7] hover:text-foreground transition-all">
                        ...
                        <button title="Delete" onClick={() => setModal({ kind: 'delete', shop })} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-red-50 hover:text-red-500 transition-all">
